...
namespace Kocmoca
{
    ...
    public class KocmocraftManager : MonoBehaviourPun
    {
        ...
        public void InitializeLocalPlayer()
        {
            ...
            myTransform.name = photonView.Owner.NickName + "-" + KocmocraftData.GetKocmocraftName((int)Type);
            // Add Search List
            ...
            // HUD - Initialize
            FindObjectOfType<HeadUpDisplayManager>().InitializeHUD(myTransform, Number);
        }
        [PunRPC]
        ...
        public void InitializeLocalBot(int portNumber)
        {
            ...
            myTransform.name = KocmocraftData.GetBotName(portNumber) + "-" + KocmocraftData.GetKocmocraftName((int)Type);
            // Add Search List
            SatelliteCommander.Instance.AddSearchList(myTransform, (int)Faction, Number);
            // Kocmonaut Info ( first time only )
            if (!SatelliteCommander.Instance.listKocmonaut.ContainsKey(Number))
