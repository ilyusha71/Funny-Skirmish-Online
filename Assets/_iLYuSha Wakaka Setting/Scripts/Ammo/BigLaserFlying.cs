    ...
    public class BigLaserFlying : Ammo
    {
        ...
        IEnumerator FlyingInitialize()
        {
            ...
            if (target)
            {
                ...
                float expectedTime = Mathf.Sqrt(nowDistance * nowDistance / (coefficient * coefficient * BigLaserCannon.flightVelocity * BigLaserCannon.flightVelocity - target.GetComponent<Rigidbody>().velocity.sqrMagnitude));
            ...
            if (raycastHits.Length > 0)
            {
                ...
                if (hull)
                {
                    ...
                    hull.Hit(new DamageInfo()
                    {
                        ...
                        Shield = (int)(basicDamage * BigLaserCannon.coefficientMaxDamage)
                    });
                }
                Recycle(gameObject);
                return;
